...
planilha = ler_arquivo_para_dataframe(bucket_name=bucket_name, s3_key=f'Cat42/{nome_empresa.title()}/Ficha 3/ficha_3_{nome_empresa.title()}_{cnpj}_v2.xlsx',
                                      file_type='xlsx')
...
planilha['N C M'] = planilha['N C M'].replace(r'[^\d]', '', regex=True)

# Substitui os valores não numéricos por NaN antes de tentar a conversão
planilha['CEST'] = planilha['CEST'].replace(r'[^\d]', '', regex=True)

# Converte os valores para numéricos, erros serão convertidos para NaN
planilha['CEST'] = pd.to_numeric(planilha['CEST'], errors='coerce')

# Converte para o tipo 'Int64' com valores nulos preservados
planilha['CEST'] = planilha['CEST'].astype('Int64')
...
planilha['QTD_CAT'] = planilha['QTD_CAT'].apply(
    lambda x: f"{x:.3f}".replace('.', ','))

planilha['COD_LEGAL'] = pd.to_numeric(planilha['COD_LEGAL'])
planilha['COD_LEGAL'] = planilha['COD_LEGAL'].astype(
    'Int64').astype(str).replace('<NA>', np.nan)

planilha['CFOP'] = pd.to_numeric(planilha['CFOP'])
planilha['CFOP'] = planilha['CFOP'].astype(
    'Int64').astype(str).replace('<NA>', np.nan)
...
cols_1100 = ['CHV_DOC', 'DATA', 'NUM_ITEM', 'IND_OPER', 'COD_ITEM', 'CFOP',
             'QTD_CAT', 'ICMS_TOT_PCAT', 'VLR_CONFR_PCAT', 'COD_LEGAL', 'FONTE']

df_1100 = planilha.copy()
df_1100 = df_1100[cols_1100]
df_1100['Data'] = df_1100['DATA']
df_1100['DATA'] = df_1100['DATA'].astype(str)
df_1100['DATA'] = [datetime.strptime(
    x, '%Y-%m-%d').strftime('%d%m%Y') for x in df_1100['DATA']]
...
df_1100 = df_1100[['COD_REG', 'CHV_DOC', 'Data', 'DATA', 'NUM_ITEM', 'IND_OPER', 'COD_ITEM', 'CFOP', 'QTD_FIN',
                   'ICMS_TOT_PCAT', 'VLR_CONFR_PCAT', 'COD_LEGAL', 'FONTE']]

df_1100['Data'] = df_1100['Data'].astype(str)
df_1100['Data'] = [datetime.strptime(
    x, '%Y-%m-%d').strftime('%m%Y') for x in df_1100['Data']]

df_1100['QTD_FIN'] = df_1100['QTD_FIN'].replace('nan', np.nan)

# Leitura do arquivo da 1050

arquivo_1050 = ler_arquivo_para_dataframe(
    bucket_name, f'Cat42/{nome_empresa.title()}/1050/1050_{nome_empresa.title()}_{cnpj}_v2.xlsx', file_type='xlsx')

mes_ano = []
for date in arquivo_1050['REF']:
    mes_ano.append(str(date)[5:7] + str(date)[0:4])

arquivo_1050['DATA'] = [datetime.strptime(x, '%Y%m').strftime(
    '%m%Y') for x in arquivo_1050['REF'].astype(str)]
...
def dataframe_to_txt_with_crlf(data, bucket_name, path):
    ...
    for index, row in data.iterrows():
        row_str = '|'.join(
            map(lambda x: '' if pd.isna(x) else str(x), row)) + '\r\n'
...
df_0150['data'] = [datetime.strptime(
    x, '%d/%m/%Y').strftime('%m%Y') for x in df_0150['data']]
df_0150['cnpj'] = df_0150['cnpj'].replace('', np.nan)
df_0150['ie'] = df_0150['ie'].replace('', np.nan)
df_0150['ie'] = df_0150['ie'].str.lstrip('\n')
df_0150['codigo_participante'] = df_0150['codigo_participante'].str.replace(
    r'^(FOR|CLI)', '', regex=True)
...
for date in dates:
    for cnpj_ in df_0150_final[df_0150_final['data'] == date]['cnpj'].unique():
        if len(df_0150_final.loc[(df_0150_final['data'] == date) & (df_0150_final['cnpj'] == cnpj_)]) > 1:
            df = df_0150_final.loc[(df_0150_final['data'] == date) & (
                df_0150_final['cnpj'] == cnpj_)]
...
if df_cnpj_repetido.shape[0] > 0:
    ...
    print(
        f'❌A data em que ocorreu a repetição é {list(df_cnpj_repetido["data"].unique())}')
    sys.exit()
...
if df_ie_repetido.shape[0] > 0:
    ...
    print(
        f'❌A data em que ocorreu a repetição é {list(df_ie_repetido["data"].unique())}')
    sys.exit()

# Remove duplicatas
df_0150_final = df_0150_final.drop_duplicates(
    subset=('codigo_participante', 'data'), keep='first')

# Verifica duplicatas diretamente no DataFrame
duplicados = df_0150_final[df_0150_final.duplicated(
    subset=['codigo_participante', 'data'], keep=False)]
...
df_0150_final['codigo_municipio'] = pd.to_numeric(
    df_0150_final['codigo_municipio'])

df_0150_final_new = df_0150_final[df_0150_final['ie'].notnull()]

df_0150_final_new['cnpj'] = df_0150_final_new['cnpj'].apply(
    lambda x: str(x).zfill(14) if len(str(x)) < 14 else str(x))

# Função para verificar o dígito verificador de um CNPJ


def verifica_digito_verificador(cnpj):
    # Converte o CNPJ em uma lista de números inteiros
    cnpj = [int(d) for d in str(cnpj) if d.isdigit()]
    if len(cnpj) != 14:
...
if len(df['Dígito_Verificador_Válido'].unique()) > 1:
    ...
    print(
        f'❌O CNPJ com dígito verificador inválido é {list(df["cnpj"].unique())}')
    sys.exit()
...
def recalcular_digito_verificador(nfe_completa):
    ...
    if pd.isnull(nfe_completa) or len(nfe_completa) != 44:
        raise ValueError(
            f"❌Valor inválido na coluna 'CHV_DOC': {nfe_completa}")
...
for date in planilha['Data'].unique():
    ...
    if df_0000_.shape[0] != 1:
        print(
            f'❌Erro no 0000 do CNPJ {cnpj} para a data {date}. Favor verificar')
        sys.exit()
    ...
    if df_0150_.shape[0] == 0:
        print(
            f'❌CNPJ {cnpj} não possui 0150 para a data {date}. Favor atualizar arquivo')
        sys.exit()
    ...
    # Geração da 0200
    df_0200 = planilha[planilha['Data'] == date][['COD_ITEM', 'DESCRICAO',
                                                  'CODIGO_BARRA', 'UNIDADE', 'N C M', 'ALIQUOTA', 'CEST']].drop_duplicates()
    df_0200['COD_REG'] = ['0200']*(df_0200.shape[0])
    df_0200 = df_0200[['COD_REG', 'COD_ITEM', 'DESCRICAO',
                       'CODIGO_BARRA', 'UNIDADE', 'N C M', 'ALIQUOTA', 'CEST']]
    ...
    filtered = arquivo_1050[arquivo_1050['DATA'] ==
                            date][arquivo_1050['COD_ITEM'].isin(unique_cod_items)]
    filtered

    df_1050 = filtered[['COD_ITEM', 'QTD_INI', 'ICMS_INI',
                        'SALDO_FINAL_MES_QTD', 'SALDO_FINAL_MES_ICMS']]

    df_1050['COD_REG'] = ['1050']*(df_1050.shape[0])
    df_1050['QTD_INI'] = df_1050['QTD_INI'].apply(
        lambda x: f"{x:.3f}".replace('.', ','))
    df_1050['ICMS_INI'] = df_1050['ICMS_INI'].astype(float).map(format_number)
    df_1050['SALDO_FINAL_MES_QTD'] = df_1050['SALDO_FINAL_MES_QTD'].apply(
        lambda x: f"{x:.3f}".replace('.', ','))
    df_1050['SALDO_FINAL_MES_ICMS'] = df_1050['SALDO_FINAL_MES_ICMS'].astype(
        float).map(format_number)

    df_1050 = df_1050[['COD_REG', 'COD_ITEM', 'QTD_INI',
                       'ICMS_INI', 'SALDO_FINAL_MES_QTD', 'SALDO_FINAL_MES_ICMS']]
    ...
    if df_1100_[df_1100_['VLR_CONFR_PCAT'] < 0.01].shape[0] > 0:
        ...
        print(mensagem)
        sys.exit()
    df_1100_['ICMS_TOT_PCAT'] = df_1100_[
        'ICMS_TOT_PCAT'].astype(float).map(format_number)
    df_1100_['VLR_CONFR_PCAT'] = df_1100_[
        'VLR_CONFR_PCAT'].astype(float).map(format_number)
    df_1100_['VLR_CONFR_PCAT'] = df_1100_[
        'VLR_CONFR_PCAT'].astype(str).replace('nan', np.nan)
    df_1100_['ICMS_TOT_PCAT'] = df_1100_[
        'ICMS_TOT_PCAT'].astype(str).replace('nan', np.nan)
    df_1100_ = df_1100_[['COD_REG', 'CHV_DOC', 'DATA', 'NUM_ITEM', 'IND_OPER', 'COD_ITEM',
                         'CFOP', 'QTD_FIN', 'ICMS_TOT_PCAT', 'VLR_CONFR_PCAT', 'COD_LEGAL', 'FONTE']]
    ...
    df_1100_['CHV_DOC'] = np.where((df_1100_['CHV_DOC'].str.slice(6, 20).isin(cnpjs)) | (df_1100_['IND_OPER'] == 1),
                                   df_1100_['CHV_DOC'].str.slice(
                                       0, 6) + cnpj_loja + df_1100_['CHV_DOC'].str.slice(20, 44),
                                   df_1100_['CHV_DOC'])

    df_1100_['DV_NFe'] = df_1100_['CHV_DOC'].apply(
        recalcular_digito_verificador)
    df_1100_['CHV_DOC'] = df_1100_['CHV_DOC'].str.slice(
        0, -1) + df_1100_['DV_NFe'].astype(str)
    ...
    dataframe_to_txt_with_crlf(data=df_0000_,
                               ...
                               path=f'Cat42/{nome_empresa.title()}/Txts/Teste_CNPJ_{cnpj}/0000_{date}_{cnpj}_v2.txt')
    print(
        f'Gerado o arquivo 0000 e salvo em Cat42/{nome_empresa.title()}/Txts/Teste_CNPJ_{cnpj}/0000_{date}_{cnpj}_v2.txt')

    # Geração do .txt da 0150
    dataframe_to_txt_with_crlf(data=df_0150_,
                               bucket_name=bucket_name,
                               path=f'Cat42/{nome_empresa.title()}/Txts/Teste_CNPJ_{cnpj}/0150_{date}_{cnpj}_v2.txt')
    print(
        f'Gerado o arquivo 0150 e salvo em Cat42/{nome_empresa.title()}/Txts/Teste_CNPJ_{cnpj}/0150_{date}_{cnpj}_v2.txt')

    # Geração do .txt da 0200
    dataframe_to_txt_with_crlf(data=df_0200,
                               bucket_name=bucket_name,
                               path=f'Cat42/{nome_empresa.title()}/Txts/Teste_CNPJ_{cnpj}/0200_{date}_{cnpj}_v2.txt')
    print(
        f'Gerado o arquivo 0200 e salvo em Cat42/{nome_empresa.title()}/Txts/Teste_CNPJ_{cnpj}/0200_{date}_{cnpj}_v2.txt')

    # Geração do .txt da 1050
    dataframe_to_txt_with_crlf(data=df_1050,
                               bucket_name=bucket_name,
                               path=f'Cat42/{nome_empresa.title()}/Txts/Teste_CNPJ_{cnpj}/1050_{date}_{cnpj}_v2.txt')
    print(
        f'Gerado o arquivo 1050 e salvo em Cat42/{nome_empresa.title()}/Txts/Teste_CNPJ_{cnpj}/1050_{date}_{cnpj}_v2.txt')
    ...
    for file in files.get('Contents', []):
        ...
        if f"{date}_{cnpj}" in file_key and file_key.endswith('v2.txt'):
            # Separar o arquivo pela data (ex: 032020)
            # Obtém a parte de data do nome do arquivo
            file_date = f"{file_key.split('_')[3]}"
    ...
    for file_date, file_list in files_by_date.items():
        ...
        s3_path = f'Cat42/{nome_empresa.title()}/Txts/cat_{nome_empresa.title()}_{cnpj}_{file_date}_v2.txt'
