...
# -- Cóidgo para leitura da tabela 2 -- #
...
def calcular_ressarcimento(tabela_2):
    ...
    tabela_2['ICMS_TOT'] = tabela_2['ICMS_TOT'].astype(float) 
    # Criação de dataset para cálculo do ICMS inicial

    data = tabela_2[['COD_ITEM', 'DATA', 'QTD_CAT', 'IND_OPER']]
    ...
    data['QTD_CAT'] = data['QTD_CAT'].astype(float)
    ...
    data['ICMS_TOT_0'] = tabela_2['ICMS_TOT'].astype(float)
    ...
    data = data.merge(grouped_data[['ICMS_INI']], how='left', left_on='COD_ITEM', right_index=True)
    ...
    ficha_3 = tabela_2[['CHV_DOC', 'DATA', 'CFOP', 'NUM_ITEM', 'COD_ITEM', 'IND_OPER', 'SUB_TIPO', 'QTD_CAT', 'ALIQUOTA', 'FONTE']]
    ...
    ficha_3['ICMS_TOT'] = ficha_3['ICMS_TOT'].fillna(0)
    ...
    mask = np.abs(ficha_3['ICMS_SAIDA']) > np.abs(ficha_3['VLR_CONFR_1'])
    vlr_ressarc = np.where(ficha_3['COD_LEGAL'] == 1, (np.where(mask, ficha_3['ICMS_SAIDA']-ficha_3['VLR_CONFR_1'], 0)),0)

    ficha_3['VLR_RESSARCIMENTO'] = vlr_ressarc
    
    mask = np.abs(ficha_3['ICMS_SAIDA']) < np.abs(ficha_3['VLR_CONFR_1'])
    vlr_compl = np.where(ficha_3['COD_LEGAL'] == 1, np.where(mask, -ficha_3['ICMS_SAIDA']+ficha_3['VLR_CONFR_1'], 0),0)

    ficha_3['VLR_COMPLEMENTO'] = vlr_compl
    
    ficha_3['COD_LEGAL_PCAT'] = np.where((ficha_3['COD_LEGAL'] == 1) & (ficha_3['VLR_RESSARCIMENTO'] != 0) & (ficha_3['ALIQUOTA'] != 0), 1, 0)
    ficha_3.loc[pd.isna(ficha_3['COD_LEGAL']), 'COD_LEGAL_PCAT'] = np.nan
    
    ficha_3['VLR_CONFR_PCAT'] = np.where(ficha_3['COD_LEGAL_PCAT'] == 1, np.abs(ficha_3['VLR_CONFR_1']), np.nan)
    if ficha_3[(ficha_3['COD_LEGAL'] == 1) & (ficha_3['VLR_CONFR_PCAT'] <= 0)].shape[0] > 0:
    ...
    ficha_3['COD_ITEM'] = tabela_2['COD_ITEM']


    ficha_3.rename(columns={'ICMS_TOT': 'ICMS_TOT_0',}, inplace=True)
    ficha_3['ICMS_TOT'] = tabela_2['ICMS_TOT']
    ...
    ficha_3 = ficha_3[['CHV_DOC', 'DATA', 'CFOP', 'NUM_ITEM', 'COD_ITEM', 'IND_OPER', 'SUB_TIPO', 'QTD_CAT',
                        ...
                        'ICMS_TOT', 'COD_ITEM', 'CNPJ EMITENTE', 'vBCST', 
                        'FONTE']]
