...
def calcular_ressarcimento(tabela_2, cnpj_produtos):
    ...
    tabela_2 = tabela_2[tabela_2.groupby('COD_ITEM')['IND_OPER'].transform(
        lambda x: not (x.nunique() == 1 and x.iloc[0] == 0))]
    ...
    ficha_3 = ficha_3.merge(produtos[['codigo_produto', 'mva_antes']],
                            ...
                            how='left')
    ficha_3 = ficha_3.drop('codigo_produto', axis=1).rename(
        columns={'mva_antes': 'MVA'})

    ficha_3['VALOR'] = ficha_3['VALOR'].astype(
        str).replace('-', np.nan).astype(float)
    ficha_3['ALIQUOTA'] = ficha_3['ALIQUOTA'].astype(
        str).replace('-', np.nan).astype(float)
    ...
    ficha_3['Valor ICMS Operação'] = np.where(cond_1 | cond_2,
                                              (ficha_3['ALIQUOTA'].astype(
                                                  float) / 100) * ficha_3['VALOR'].astype(float),
    ...
    data = data.sort_values(
        by=['COD_ITEM', 'DATA', 'IND_OPER'], ascending=[True, True, True])

    # Cáculo dos saldos das operações para definição da Quantidade Inicial
    data['OPER'] = np.where(data['IND_OPER'] == 0,
                            data['QTD_CAT'], -data['QTD_CAT'])
    ...
    data['QTD_INI'] = np.where(data.index.isin(
        first_rows.index), data['QTD_INI'], np.nan)
    ...
    data['VALOR_UNIT'] = np.where(data['IND_OPER'] == 0,
                                  data['ICMS_TOT_0'].fillna(
                                      0) / data['QTD_CAT'],
                                  np.nan)
    data['VALOR_OP_UNIT'] = np.where(data['IND_OPER'] == 0,
                                     data['Valor ICMS Operação'].fillna(
                                         0) / data['QTD_CAT'],
                                     np.nan)
    ...
    prods_icms_0 = data.drop_duplicates(subset='COD_ITEM', keep='first')[(data['QTD_INI'] != 0) & (
        ((data['ICMS_INI'] == 0) | (data['ICMS_INI'].isnull())) | ((data['ICMS_OP_INI'] == 0) | (data['ICMS_OP_INI'].isnull())))]
    prods_icms_ini_igual_op = data.drop_duplicates(subset='COD_ITEM', keep='first')[
        data['ICMS_INI'] == data['ICMS_OP_INI']]
    for produto in list(produtos_somente_saida) + list(prods_icms_0['COD_ITEM'].unique()) + list(prods_icms_ini_igual_op['COD_ITEM'].unique()):
        ...
        lucro = sum(ficha_3[(ficha_3['COD_ITEM'] == produto) & (
            ficha_3['IND_OPER'] == 1)]['ICMS_TOT_SAIDA'])

        tot_qtde = sum(ficha_3[(ficha_3['COD_ITEM'] == produto) & (
            ficha_3['IND_OPER'] == 1)]['QTD_CAT'])

        aliquota = ficha_3[ficha_3['COD_ITEM'] ==
                           produto]['ALIQUOTA'].values[0] / 100

        qtde_inicial = data[data['COD_ITEM'] ==
                            produto]['QTD_INI'].iloc[0]
        ...
        if not produto_idx.empty:
            ...
            data.loc[produto_idx[0], 'ICMS_OP_INI'] = icms_op_unit * \
                data.loc[produto_idx[0], 'QTD_INI']
    ...
    ficha_3['ICMS_EFETIVO_ENTRADA'] = ficha_3['valor_medio'] * \
        ficha_3['QTD_CAT']
    ...
    qtd_ent1_devolv_ent = list(np.where(
        mask_condition1, ficha_3['QTD_CAT'], 0) + np.where(mask_condition2, -ficha_3['QTD_CAT'], 0))
    ficha_3['QTD_ent1_devolv_ent'] = qtd_ent1_devolv_ent

    mask = ficha_3['QTD_ent1_devolv_ent'] > 0

    ficha_3['ICMS_TOT_ent_unit'] = np.where(
        mask, ficha_3['ICMS_TOT'].fillna(0) / ficha_3['QTD_ent1_devolv_ent'], 0)
    ...
    qtd_saida_1_devolv_saida_t = np.where(
        mask_condition1, ficha_3['QTD_CAT'], 0) + np.where(mask_condition2, -ficha_3['QTD_CAT'], 0)
    ...
    ficha_3['REF'] = [datetime.strptime(
        x, '%Y-%m-%d').strftime('%m-%Y') for x in ficha_3['REF']]
    ...
    for code in ficha_3['COD_ITEM'].unique():
        ...
        icms_tot_saldo_int = []
        icms_tot_1_int = []
        qtd_saida_1_devolv_saida = list(
            ficha_3[ficha_3['COD_ITEM'] == code]['qtd_saida_1_devolv_saida'])
            ...
            if i == 0:
                if qtd_saida_1_devolv_saida[0] > 0:
                    if row['QTD_INI'] == 0:
                        ...
                        icms_saida_uni_int.append(
                            row['ICMS_INI'] / row['QTD_INI'])
                else:
                    icms_saida_uni_int.append(0)
                ...
                if (qtd_saida_1_devolv_saida[i] != 0) and (qtd_saldo[i-1] != 0):
                    icms_saida_uni_int.append(
                        icms_tot_saldo_int[i-1] / qtd_saldo[i-1])
                else:
                    icms_saida_uni_int.append(0)
        ...
        # ICMS_SAIDA
            if qtd_saida_1_devolv_saida[i] < 0:
                icms_saida_int.append(
                    qtd_saida_1_devolv_saida[i]*ult_icms_saida_uni_int[i])
            else:
                icms_saida_int.append(
                    icms_saida_uni_int[i]*qtd_saida_1_devolv_saida[i])
            ...
            if i == 0:
                ...
                if row['QTD_ent1_devolv_ent'] < 0:
                    if (qtd_cat[i] == qtd_saldo[i-1]):
                        ...
                        icms_tot_1_int.append(max(
                            row['QTD_ent1_devolv_ent'] * row['ULT_ICMS_TOT_ent_unit'], -1*icms_tot_saldo_int[i-1]))
                else:
                    if pd.isna(row['ICMS_TOT']):
                        icms_tot_1_int.append(0)
                    else:
        ...
        # ICMS_TOT_SALDO
            if i == 0:
                icms_tot_saldo_int.append(
                    row['ICMS_INI'] + icms_tot_1_int[i] - icms_saida_int[i])
            else:
                icms_tot_saldo_int.append(
                    icms_tot_saldo_int[i-1] + icms_tot_1_int[i] - icms_saida_int[i])
    ...
    vlr_confr_unit = np.where(
        mask, ficha_3['VLR_CONF_0']/ficha_3['qtd_saida_1_devolv_saida'], np.nan)
    ...
    ficha_3['VLR_CONFR_1'] = np.where(ficha_3['qtd_saida_1_devolv_saida'] > 0,
                                      ...
                                      np.where(ficha_3['qtd_saida_1_devolv_saida'] < 0,
                                               ficha_3['ULT_VLR_CONFR_UNIT'] *
                                               ficha_3['qtd_saida_1_devolv_saida'],
                                               0))
    ...
    ficha_3['VLR_CONFR_1'] = np.where(
        cond_1 | cond_2, ficha_3['VLR_CONFR_1'], np.nan)
    ...
    ficha_3['VLR_RESSARCIMENTO'] = np.where(ficha_3['CFOP'].isin(
        apenas_movimentacao), 0, ficha_3['VLR_RESSARCIMENTO'])
    ...
    ficha_3['VLR_RESSARCIMENTO'] = np.where((ficha_3['COD_LEGAL'] == 2) & (~ficha_3['CFOP'].isin(apenas_movimentacao)) & (~ficha_3['CFOP'].isin([2202, 5117, 5120, 5929])),
                                            ficha_3['ICMS_EFETIVO_ENTRADA'].fillna(
                                                0),
                                            ficha_3['VLR_RESSARCIMENTO'])
    ...
    ficha_3['VLR_RESSARCIMENTO'] = np.where((ficha_3['COD_LEGAL'] == 4) & (~ficha_3['CFOP'].isin(apenas_movimentacao)) & (~ficha_3['CFOP'].isin([2202, 5117, 5120, 5929])),
                                            np.abs(
                                                ficha_3['ICMS_SAIDA'] - ficha_3['ICMS_EFETIVO_ENTRADA'].fillna(0)),
                                            ficha_3['VLR_RESSARCIMENTO'])

    # Garantir que o cálculo de 'VLR_RESSARCIMENTO' seja 0 para as condições específicas
    ficha_3['VLR_RESSARCIMENTO'] = np.where(
        (ficha_3['CFOP'].isin([2202, 5117, 5120, 5929])) & (
            ficha_3['CST'].astype(float) != 60),
        0,
        ficha_3['VLR_RESSARCIMENTO']
    )
    ...
    ficha_3['COD_LEGAL'] = np.where((ficha_3['COD_LEGAL'] == 1) & (
        ficha_3['VLR_RESSARCIMENTO'] != 0) & (ficha_3['ALIQUOTA'] != 0), 1, ficha_3['COD_LEGAL'])
    ficha_3['COD_LEGAL'] = np.where(ficha_3['CFOP'].isin(
        [5404, 5403, 5401, 5405]), 1, ficha_3['COD_LEGAL'])
    ficha_3['COD_LEGAL'] = np.where(ficha_3['CFOP'].isin(
        [5409, 1411, 2411, 1202, 2202]), 0, ficha_3['COD_LEGAL'])
    ficha_3['COD_LEGAL'] = np.where(ficha_3['CFOP'].isin(
        [1403, 1409, 1102, 2102, 2403, 1409, 5411, 6411, 2409, 5202, 6202]), np.nan, ficha_3['COD_LEGAL'])
    ficha_3['COD_LEGAL'] = np.where((ficha_3['VLR_COMPLEMENTO'] > 0) & (ficha_3['DATA'] <= '2020-12-31'),
                                    0,
                                    ficha_3['COD_LEGAL'])
    ...
    mask = (ficha_3['COD_ITEM'].shift(-1) != ficha_3['COD_ITEM']
            ) | (ficha_3['REF'].shift(-1) != ficha_3['REF'])
